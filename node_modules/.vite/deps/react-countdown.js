...
//#endregion
//#region node_modules/react-countdown/dist/index.es.js
var import_react = /* @__PURE__ */ __toESM(require_react());
var import_prop_types = require_prop_types();
...
function calcTimeDelta(date) {
	...
	var startTimestamp;
	if (typeof date === "string") startTimestamp = new Date(date).getTime();
	else if (date instanceof Date) startTimestamp = date.getTime();
	else startTimestamp = date;
	if (!controlled) startTimestamp += offsetTime;
...
var Countdown = function(_React$Component) {
	...
	function Countdown() {
		...
		_this.state = { count: _this.props.count || 3 };
		_this.startCountdown = function() {
			_this.interval = window.setInterval(function() {
				if (_this.state.count - 1 === 0) {
					_this.stopCountdown();
		...
		_this.addTime = function(seconds) {
			...
			_this.setState(function(prevState) {
				return { count: prevState.count + seconds };
	...
	_createClass(Countdown, [
		...
		},
		{
			...
			value: function render() {
				return this.props.children ? (0, import_react.cloneElement)(this.props.children, { count: this.state.count }) : null;
			}
		}
	]);
	return Countdown;
...
var Countdown$1 = function(_React$Component) {
	...
	function Countdown$1(props) {
		...
		_this.tick = function() {
			...
			var callback = timeDelta.completed && !_this.props.overtime ? void 0 : _this.props.onTick;
			_this.setTimeDeltaState(timeDelta, void 0, callback);
		};
	...
	_createClass(Countdown$1, [
		...
		},
		{
			...
			value: function setTimeDeltaState(timeDelta, status, callback) {
				...
				if (completing && !this.props.overtime) this.clearTimer();
				return this.setState(function(prevState) {
					var newStatus = status || prevState.status;
					if (timeDelta.completed && !_this2.props.overtime) newStatus = "COMPLETED";
					else if (!status && newStatus === "COMPLETED") newStatus = "STOPPED";
					return {
						timeDelta,
						status: newStatus
					};
					...
					if (_this2.props.onComplete && (completing || completedOnStart)) _this2.props.onComplete(timeDelta, completedOnStart);
				});
			}
		},
		{
		...
		},
		{
			...
			value: function render() {
				...
				if (children && this.state.timeDelta.completed && !overtime) return (0, import_react.cloneElement)(children, { countdown: renderProps });
				...
				return (0, import_react.createElement)("span", { className }, renderProps.total < 0 ? "-" : "", days, days ? ":" : "", hours, ":", minutes, ":", seconds);
			}
		}
	]);
	return Countdown$1;
...
Countdown$1.propTypes = {
	date: (0, import_prop_types.oneOfType)([
		(0, import_prop_types.instanceOf)(Date),
		import_prop_types.string,
		import_prop_types.number
	]),
...
//#endregion
export { calcTimeDelta, Countdown$1 as default, formatTimeDelta, zeroPad };
